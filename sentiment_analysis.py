...
def vader_sentiment_analysis(cleaned_path: str, analysis_path: str):
    ...
    for df in pd.read_csv(cleaned_path, chunksize=csv_chunksize):
        df["sentiment"] = df["text"].apply(lambda x: vader_extract_score(model, x))
        df.to_csv(analysis_path, index=False, header=False, mode="a", escapechar='\\')
...
def bert_sentiment_analysis(cleaned_path: str, analysis_path: str):
    ...
    df = pd.DataFrame(columns=columns_sentiment)
    df.to_csv(analysis_path, index=False)
    for i, chunk in enumerate(pd.read_csv(cleaned_path, chunksize=csv_chunksize)):
        print("Chunk", i)
        chunk["sentiment"] = bert_extract_score(model, chunk["text"].tolist())
        chunk.to_csv(analysis_path, index=False, header=False, mode="a", escapechar='\\')
